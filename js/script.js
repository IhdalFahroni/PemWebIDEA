// Menunggu sampai seluruh HTML siap sebelum menjalankan script
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    const heroElement = document.querySelector(".hero"); // Referensi ke div.hero
    const heroTextElement = document.getElementById("hero-text");
    const transitionDuration = 500; // Samakan dengan durasi transisi opacity di CSS (ms)
    const slideInterval = 5000; // Interval total (ms)

    if (headerElement && heroElement && heroTextElement) {

        function changeSlide() {
            // 1. Mulai Fade Out Teks
            ...
            setTimeout(() => {
                ...
                // 3. Ubah Background (CSS akan handle transisinya jika ada)
                headerElement.style.backgroundImage = currentSlide.image;

                // 4. Ubah Teks (saat masih transparan)
                heroTextElement.innerHTML = currentSlide.text;

                // 5. Ubah Posisi Teks (dengan menghapus kelas lama & menambah baru)
                heroElement.classList.remove(...allPositionClasses); // Hapus semua kelas posisi
                heroElement.classList.add(...currentSlide.positionClass.split(' ')); // Tambah kelas baru

                // 6. Mulai Fade In Teks
                heroTextElement.classList.remove('opacity-0');

                // 7. Pindah ke indeks berikutnya
                currentIndex = (currentIndex + 1) % slidesData.length;

            }, transitionDuration); // Waktu tunggu = durasi fade-out
        }

        // Panggil pertama kali & set interval
        changeSlide(); 
        // Set interval sedikit lebih lama dari durasi transisi untuk mencegah tumpang tindih
    ...
    // Script untuk rating bintang (jika Anda memindahkannya juga)
    document.querySelectorAll('.rating').forEach(el => {
        ...
        el.innerHTML = starsHTML + ` <span class="ml-1">(${rating.toFixed(1)})</span>`;
    });
